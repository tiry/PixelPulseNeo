...
def format_date_short() -> str:
    # Get the current date and time
    current_datetime: datetime = datetime.now()
    date_str: str = current_datetime.strftime(f"%m/%d/%y")
...
class BaseCommand:
    ...
    def execute(self, stop_event: threading.Event, timeout: int=10, args:list=[], kwargs:dict={}, render:bool=True) -> tuple | None:
        t0: float = time.time()
        try:
            logger.debug(
...
class PictureScrollBaseCmd(MatrixBaseCmd):
    ...
    def _resize_icon(self, icon: Image.Image, max_height:int|None = None , max_width:int|None= None ) -> Image.Image:
        """
